...
chapters = agent.run('Output the novel chapter names and summaries for each chapter, return as list [(chapter_title, chapter_summary), ....]', return_type=list)

# Step 4: Generate detailed content for each chapter
contents = []
for index, (chapter_title, chapter_summary) in enumerate(chapters):
    content = agent.run(f'For chapter: {chapter_title}\n{chapter_summary}. \nOutput detailed chapter content, return content only without title.')
